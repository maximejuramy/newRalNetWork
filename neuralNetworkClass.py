...
# Pb with cross-entropy => review definition and implementation
# Comment + factorize + separate in ≠ files
# Check with MNIST
...
def cross_entropy(predicted_scores, true_labels):
    if len(true_labels) == 1:
        raise ValueError("Well you cannot really use Cross-Entropy for a regression problem I'm afraid")

    # Only one class equals 1 (all others equal 0) => so by taking the scalar product and then 
...
class TwoLayerNeuralNetwork(object):
    def __init__(self, input_length, hidden_layer_length, output_length, activation_function):
        ...
        self.hidden_layer_weights = np.random.normal(
            loc=0.0, scale=hidden_layer_initialization_std, size=(self.hidden_layer_length, self.input_length)
        )
        ...
        self.output_layer_weights = np.random.normal(
            loc=0.0, scale=output_layer_initialization_std, size=(self.output_length, self.hidden_layer_length)
        )
    ...
    def feed_forward(self, input):
        ...
        output_activations = self.output_activation_function(
            self.output_layer_weights.dot(hidden_layer_activations) + self.output_layer_bias
        )
    ...
    def back_propagate(self, activations, input, output):
        """
            This functions returns the gradients of the weights and biases of the network
            computed from the given activations (that come from a previous feed forward)
        """

        # Handling the output layer
        ############################
        z_output = (
            self.output_layer_weights.dot(activations[0].reshape(self.hidden_layer_length,1)) + 
            self.output_layer_bias.reshape(self.output_length,1)
        )

        # Gradient of the output layer bias
        nabla_b_output_layer = (
             (2*(activations[-1] - output)).reshape(self.output_length,1) * softmax(z_output)
        )
        ...
        nabla_b_hidden_layer = (
            self.output_layer_weights.transpose().dot(nabla_b_output_layer) * softmax(z_hidden)
        )
        ...
        return nabla_w_hidden_layer, nabla_b_hidden_layer, nabla_w_output_layer, nabla_b_output_layer
    ...
    def SGD(self, inputs, outputs, epochs, eta, mini_batch_size):
        ...
        for epoch in range(epochs):
            ...
            print(score/n)
            print("The cross-entropy at the end of this epoch is {}\n".format(round(score/len(training_data),1)))


    def update_mini_batch(self, batch, eta):
        """
            This method takes a batch of (input, label)'s and updates (once) the weights
            and biases of the network
        """
        nabla_w_hidden_layer, nabla_b_hidden_layer, nabla_w_output_layer, nabla_b_output_layer = (
            [], [], [], []
        )
        for (x, y) in batch:
            # Feed forward
            ...
            ) =  self.back_propagate(current_activations, np.array(x), np.array(y))
        ...
        delta_nabla_b_hidden_layer = (sum(nabla_b_hidden_layer)/len(batch)).reshape(self.hidden_layer_length,1)
        delta_nabla_w_output_layer = sum(nabla_w_output_layer)/len(batch)
        delta_nabla_b_output_layer = (sum(nabla_b_output_layer)/len(batch)).reshape(self.output_length,1)

        self.hidden_layer_weights -= eta*delta_nabla_w_hidden_layer
        self.hidden_layer_bias -= (eta*delta_nabla_b_hidden_layer).reshape(self.hidden_layer_length,)
        self.output_layer_weights -= eta*delta_nabla_w_output_layer
        self.output_layer_bias -= (eta*delta_nabla_b_output_layer).reshape(self.output_length,)
